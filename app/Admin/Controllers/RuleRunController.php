...
class RuleRunController extends Controller
{
    ...
    public function index(Request $request)
    {
        if (isset($request->type)) {
            ...
            if ($type == 'gurl') {
                $typeId = $request->id;
                $rest = $this->_gurl($typeId);
            } elseif ($type == 'rule') {
                //获得当前采集链接下对应的规则,一对一
                $gurlId = $request->id;
                $rest = $this->_rule($gurlId);
            }elseif ($type == 'command'){
                $ruleId = $request->id;
...
//                    $form->action(admin_url('rule_run'));
                    ...
                    $form->select('arctype', '分类名称')->options(Arctype::selectOptions())
                        ->help('对参数--queue说明:list(列表页)content(内容页)pic(图片)dede(部署上线)cdn(一传文件)all(一键运行)');
                    //不同的按扭怎么做
     ...
     * @return string
     */
    public function _gurl($typid)
    {
        $str = '';
        $gurls = Gurl::where('arctype_id', $typid)->get();


        foreach ($gurls as $key => $value) {
            $str .= ' <option value="' . $value->id . '"> typeid -- '. $value->arctypes->dede_id .' -- '. $value->rules->rule_name . '--' . $value->gurl . '</option>';
        }

        if (empty($str) === false) {
            $str = '<div class="form-group 1"><label for="gurl" class="col-sm-2 control-label">采集网址</label><div class="col-sm-8"><select class="form-control gurl" style="width: 100%;" name="gurl"><option value="0" selected>Root</option>' . $str . '</select></div></div>';
        }
        return $str;
    }
     ...
     * @return string
     */
    public function _rule($gurlid)
    {
        $str = '<div class="rule">';
        $argStr = '';
        $ruleStr = '';
        $rule = Rule::where('gurl_id', $gurlid)->first();
        if ($rule) {
            //取出参数
            $args = $rule->rule_args;
            $args = explode(',', $args);
